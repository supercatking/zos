...
USER_MALLOCTEST_ELF := $(BUILD_DIR)/user/bin/malloctest.elf
USER_MALLOCTEST_OBJ := $(BUILD_DIR)/user/bin/malloctest_elf.o
USER_BADTEST_ELF := $(BUILD_DIR)/user/bin/badtest.elf
USER_BADTEST_OBJ := $(BUILD_DIR)/user/bin/badtest_elf.o
USER_RM_ELF := $(BUILD_DIR)/user/bin/rm.elf
USER_RM_OBJ := $(BUILD_DIR)/user/bin/rm_elf.o
USER_MKDIR_ELF := $(BUILD_DIR)/user/bin/mkdir.elf
USER_MKDIR_OBJ := $(BUILD_DIR)/user/bin/mkdir_elf.o
USER_RMDIR_ELF := $(BUILD_DIR)/user/bin/rmdir.elf
...
$(USER_FORKTEST_OBJ): $(USER_FORKTEST_ELF)
	$(LD) -m elf32lriscv -r -b binary -o $@ $<
...
$(USER_VMTEST_OBJ): $(USER_VMTEST_ELF)
	$(LD) -m elf32lriscv -r -b binary -o $@ $<
...
$(USER_MULTIFORKTEST_OBJ): $(USER_MULTIFORKTEST_ELF)
	$(LD) -m elf32lriscv -r -b binary -o $@ $<
...
$(USER_MALLOCTEST_ELF): $(BUILD_DIR)/user/bin/malloctest.o user/linker.ld
	$(CC) $(ARCH_FLAGS) $(LDFLAGS_USER) -T user/linker.ld $(BUILD_DIR)/user/bin/malloctest.o -o $@

$(USER_MALLOCTEST_OBJ): $(USER_MALLOCTEST_ELF)
	$(LD) -m elf32lriscv -r -b binary -o $@ $<

$(USER_BADTEST_ELF): $(BUILD_DIR)/user/bin/badtest.o user/linker.ld
	$(CC) $(ARCH_FLAGS) $(LDFLAGS_USER) -T user/linker.ld $(BUILD_DIR)/user/bin/badtest.o -o $@

$(USER_BADTEST_OBJ): $(USER_BADTEST_ELF)
	$(LD) -m elf32lriscv -r -b binary -o $@ $<

$(USER_RM_ELF): $(BUILD_DIR)/user/bin/rm.o user/linker.ld
	$(CC) $(ARCH_FLAGS) $(LDFLAGS_USER) -T user/linker.ld $(BUILD_DIR)/user/bin/rm.o -o $@

$(USER_RM_OBJ): $(USER_RM_ELF)
	$(LD) -m elf32lriscv -r -b binary -o $@ $<

$(USER_MKDIR_ELF): $(BUILD_DIR)/user/bin/mkdir.o user/linker.ld
	$(CC) $(ARCH_FLAGS) $(LDFLAGS_USER) -T user/linker.ld $(BUILD_DIR)/user/bin/mkdir.o -o $@

$(USER_MKDIR_OBJ): $(USER_MKDIR_ELF)
	$(LD) -m elf32lriscv -r -b binary -o $@ $<

$(USER_RMDIR_ELF): $(BUILD_DIR)/user/bin/rmdir.o user/linker.ld
	$(CC) $(ARCH_FLAGS) $(LDFLAGS_USER) -T user/linker.ld $(BUILD_DIR)/user/bin/rmdir.o -o $@

$(USER_RMDIR_OBJ): $(USER_RMDIR_ELF)
	$(LD) -m elf32lriscv -r -b binary -o $@ $<

$(USER_MV_ELF): $(BUILD_DIR)/user/bin/mv.o user/linker.ld
	$(CC) $(ARCH_FLAGS) $(LDFLAGS_USER) -T user/linker.ld $(BUILD_DIR)/user/bin/mv.o -o $@
...
disk-regression: build disk-image
	QEMU_SMOKE_INPUT='mkdir /disk/tmp\nmkdir /disk/other\necho hi > /disk/tmp/a\nls /disk\nls /disk/tmp\ncat /disk/tmp/a\nmv /disk/tmp /disk/new\nmv /disk/tmp/a /disk/other/b\ncat /disk/other/b\nstat /disk/BIG\nwc /disk/BIG\nrm /disk/other/b\nrmdir /disk/tmp\nrmdir /disk/other\nls /disk\nreboot\n' \
	QEMU_SMOKE_EXPECT='virtio-blk: queue ready;diskfs: mounted at /disk;tmp;other;a;hi;mv: failed;disk file /disk/BIG size=1191;lines=1 words=70 bytes=1191;README;BIG;user: halted cleanly' \
