...
regression: build
	...
	QEMU_SMOKE_EXPECT='commands:;echo;hello;forktest: child saw 0;forktest: wait reaped child;multifork: wait reaped 3;multifork: ok;vmtest: isolation ok;schedtest: wait reaped 3;schedtest: ok;multiforktest;schedtest;console;char /dev/console;status;meminfo;uptime;pid: 1 ppid: 0 state: running name: sh;mem total_pages=;uptime ;pid: 1;a;ZOS README;lines=1 words=1 bytes=6;pid: 1 ppid: 0 state: running name: sh;PATH=/bin;history;user: halted cleanly' \
	./scripts/run-qemu-smoke.sh $(KERNEL_ELF) $(OPENSBI_RV32)
