...
The host tool `scripts/mkfs_zos.py` creates `build/zos.img` with root
`/README` and `/BIG` files. `/BIG` spans multiple direct data sectors and is
used by `make disk-regression`.
...
The current disk filesystem supports a small fixed inode table, files up to four
direct 512-byte sectors, root-level directories, files inside those directories,
